...
from rapidcli.protos.security_factors.authn_system_enums_pb2 import (
    AuthnSystemProviderEnum,
)
...
class TestUtils(unittest.TestCase):
    def test_choice_from_proto(self):
        with patch("builtins.input", return_value="2"):
            choice = utils.select_choice_from_proto(
                ChannelGroup.ChannelType.DESCRIPTOR.values
            )
    ...
    def test_iterate_down_to(self):
        ...
        self.assertEqual(
            utils.iterate_down_to(test_data, "brolo", "nonexistent_key"), None
        )
        self.assertEqual(
            utils.iterate_down_to(test_data, "brolo", "nested_list", "key"),
            ["value1", "value2"],
        )
