...
class TestUtils(unittest.TestCase):
    def setUp(self) -> None:
        cli_config_path = os.path.join(
            pathlib.Path(__file__).parent.resolve(), "cli_config.yml"
        )
        apps_path = pathlib.Path(__file__).parent.resolve().parent.resolve()
        self.cli_config_yaml_data = load_yaml(cli_config_path)
        self.cli_config = create_cli_config_from_cli_from_data(
            self.cli_config_yaml_data, apps_path
        )
    ...
    def test_input_map_created(self):
        self.assertEqual(
            type(self.cli_config.test_extension.input_menu), InputMenuConfig
        )
    ...
    def test_nested_obj_in_list_created(self):
        self.assertEqual(
            type(
                self.cli_config.test_extension.nested_obj_in_extension.tables[
                    0
                ].nested_obj_in_list
            ),
            NestedObjInListConfig,
